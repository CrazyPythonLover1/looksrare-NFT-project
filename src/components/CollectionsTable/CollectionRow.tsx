      ...
      >
        ...
        <GridItem>
          <Flex alignItems="start" flexDirection="column">
            {!!floorPrice ? <Amount amount={formatToSignificant(floorPrice, 4)} /> : "-"}
            {!!floorChange24h && <PercentChangeLabel value={floorChange24h} textStyle="helper" />}
          </Flex>
        </GridItem>
        <GridItem>
          <AmountVolumeDisplay amount={volume.volume24h} change={volume.change24h} />
        </GridItem>
        <GridItem>{volume.volumeAll ? <Amount amount={formatToSignificant(volume.volumeAll, 0)} /> : "-"}</GridItem>
        <GridItem>
          {countOwners ? (
